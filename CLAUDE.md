...
**品牌名**：**Mova Craft**（散文 / 法律 / 邮件正文写 `Mova Craft`；logo wordmark / page title / nav 写 `MOVA CRAFT` 全大写。中文页面同样使用英文原名，不配中文名）
...
**SKU 矩阵**：
- **主产品（牌）**：v1 仅盾形 × 2 表面处理（钛本色 + 枪黑色）= **2 SKU**，统一定价 **$109 USD**。八角 / 圆形 + 香槟金 / 钛蓝 / 玫瑰铜 代码保留，UI 暂隐藏，v1.5 解锁后扩展到 15 SKU。
...
- **表面处理**（同一钛合金基材，designer A 步选择）：
  - **Titanium Silver / 钛本色**（钛合金本色抛光 — designer 默认）— v1 上线
  - **Gunmetal / 枪黑色**（DLC 涂层）— v1 上线
  - **Champagne Gold / 香槟金**（浅暖金 PVD）— 代码保留，UI 隐藏（v1.5）
  - **Sky Blue / 钛蓝**（钛合金阳极氧化标志色）— 代码保留，UI 隐藏（v1.5）
  - **Rose Copper / 玫瑰铜**（玫瑰铜 PVD）— 代码保留，UI 隐藏（v1.5）
